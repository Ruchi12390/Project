...
function AddStudent() {
  ...
  const handleChange = (event) => {
    ...
    if (name === 'admission') setAdmission(value);
  ...
  const handleSubmit = async (event) => {
    ...
    if (!firstName || !lastName || !year || !admission || !enrollment || !selectedSemester) {
      setSubmitStatus('Please fill in all fields');
      return;
    }

    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/add-student`, {
        firstName,
        lastName,
        year,
        admission,
